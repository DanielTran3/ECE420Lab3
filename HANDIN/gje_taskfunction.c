...
int main(int argc, char *argv[]) {
	...
	if (size == 1) {
		...
		{
			// Only one thread assign tasks, and threads do not have to wait
...
void gaus_elim(double** initial_mat, int* row_index, int size) {
	...
	// Find Maximum of the diagonal index and column
	find_max_column(initial_mat, row_index, size);

	// Elimination of variables under the diagonal (Upper Triangle)
...
void jord_elim(double** initial_mat, int* row_index, int size) {
	...
	for (diagonal_index = size - 1; diagonal_index > 0; diagonal_index--) {
		...
		// Create task with private variables row_elim_index, factorial_elim
		# pragma omp task private(row_elim_index, factorial_elim)
		{		
...
void store_result(double **initial_mat, double* result_mat, int* row_index, int size) {
	...
	// Create task with private variable i
...
void find_max_column(double** initial_mat, int* row_index, int size) {
	...
	// Arbitary loop variables and max values
	int diagonal_index, row, temp, temp_row_index_max;

	// Create task with private variables diagonal_index, temp_row_index_max, temp, row
	# pragma omp task private(diagonal_index, temp_row_index_max, temp, row)
	{
		...
		for (diagonal_index = 0; diagonal_index < size - 1; diagonal_index++){
			...
			// Create task with private variable row
			# pragma omp task private(row)
			{		
...
void elim_under_diag(double** initial_mat, int* row_index, int size) {
	...
	for (diagonal_index = 0; diagonal_index < size - 1; diagonal_index++){
		...
		// Cycle in for loop to through the rows below the current one
		for (row_elim_index = diagonal_index + 1; row_elim_index < size; row_elim_index++) {
			factorial_elim = initial_mat[row_index[row_elim_index]][diagonal_index] / 
							 initial_mat[row_index[diagonal_index]][diagonal_index];
	
			// Create task with private variable column_elim_index
			# pragma omp task private(column_elim_index)
			{	
